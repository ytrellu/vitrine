...
DNS_DATA=`curl -s -X GET -H "Authorization: sso-key ${GO_DADDY_API_KEY}" "https://api.godaddy.com/v1/domains/${DOMAIN}/records/A/${HOST_NAME}"`
...
if [ "$STORED_IP" != "$CURRENT_IP" ]; then
    ...
    curl -s -X PUT "https://api.godaddy.com/v1/domains/${DOMAIN}/records/A/${HOST_NAME}" \
         -H "Authorization: sso-key ${GO_DADDY_API_KEY}" \
